...
<head>
    ...
    <title>Appointment History | PUPBC CareLink</title>
    ...
    <style>
        ...
        /* Appointment ID badge style (matching student cancel class) */
    ...
    <div class="form-content">
        ...
        <div class="search-section">
            <div class="search-wrapper">
                ...
                <input type="text" class="search-box" id="searchAppointmentsInput" placeholder="Search appointment history...">
            </div>
        </div>

        <!-- COUNTER + PAGINATION ROW -->
        <div class="counter-pagination-row">
            <div class="total-counter" id="totalAppointmentCounter">
        ...
        <!-- TABLE CONTAINER - NEW COLUMN: Appointment ID between Student Email and Service Type -->
...
<!-- EDIT MODAL -->
<div class="modal fade top-aligned-modal" id="editAppointmentModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 24px;">
            <div class="modal-header" style="background: var(--brand-primary); color: white; border-radius: 24px 24px 0 0;">
                <h5 class="modal-title"><i class="fa-regular fa-pen-to-square me-2"></i>Edit Appointment Details</h5>
            ...
            <div class="modal-body modal-body-custom">
                ...
                <div class="modal-form-grid">
                    <div>
                        ...
                        <input type="text" class="form-control" id="editPatientName" placeholder="e.g., Juan Dela Cruz" style="border-radius: 12px;">
                    </div>
                    <div>
                        <label class="form-label-custom">Student ID <span class="required-star">*</span></label>
                        <input type="text" class="form-control" id="editPatientId" placeholder="e.g., STUD-2024-xxxx" style="border-radius: 12px;">
                    </div>
                    <div>
                        <label class="form-label-custom">Email Address</label>
                        <input type="email" class="form-control" id="editEmail" placeholder="student@example.com" style="border-radius: 12px;">
                    </div>
                    <div>
                        <label class="form-label-custom">Appointment ID</label>
                        <input type="text" class="form-control" id="editAppointmentIdDisplay" readonly style="border-radius: 12px; background: var(--clay-bg);">
                    </div>
                    <div>
                        <label class="form-label-custom">Appointment Time <span class="required-star">*</span></label>
                        <input type="time" class="form-control" id="editTime" style="border-radius: 12px;">
                    </div>
                    <div>
                        <label class="form-label-custom">Service Type</label>
                        <select class="form-select" id="editService" style="border-radius: 12px;">
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content" style="border-radius: 16px;">
            ...
            <div class="modal-body" style="padding: 1.5rem;">
                <p class="mb-0">Are you sure you want to permanently remove the appointment for <strong id="deleteAppointmentName"></strong>?</p>
...
<script>
    // ============================================================
    // NURSE APPOINTMENT HISTORY - WITH APPOINTMENT ID COLUMN
    // - New column "Appointment ID" inserted between Student Email and Service Type
    // - Each row includes APT-XXXX-XXX format with icon (matching student cancel class)
    // - Sample IDs: APT-2604-334, APT-2606-335, APT-2607-336, etc.
    // - Maximum 4 rows per page, pagination at top, search across all fields including Appointment ID
    // ============================================================
    
    let appointmentsList = [
        { id: "a1", appointmentId: "APT-2604-334", time: "08:30 AM", patientName: "Mark Anthony Sy", patientId: "STUD-2024-0552", email: "mark.sy@example.com", service: "Regular Check-up", status: "Completed" },
        { id: "a2", appointmentId: "APT-2606-335", time: "10:15 AM", patientName: "Maria Clara", patientId: "STUD-2023-1023", email: "maria.clara@example.com", service: "Lab Result Review", status: "Completed" },
        { id: "a3", appointmentId: "APT-2607-336", time: "01:00 PM", patientName: "Jose Rizal", patientId: "STUD-2023-0881", email: "jose.rizal@example.com", service: "Vaccination", status: "Cancelled" },
        { id: "a4", appointmentId: "APT-2608-337", time: "03:30 PM", patientName: "Andres Bonifacio", patientId: "STUD-2022-0456", email: "andres.bonifacio@example.com", service: "Dental Consultation", status: "Completed" },
        { id: "a5", appointmentId: "APT-2609-338", time: "09:00 AM", patientName: "Gabriela Silang", patientId: "STUD-2024-0789", email: "gabriela.silang@example.com", service: "Follow-up", status: "Completed" },
        { id: "a6", appointmentId: "APT-2610-339", time: "11:20 AM", patientName: "Emilio Aguinaldo", patientId: "STUD-2023-1123", email: "emilio.aguinaldo@example.com", service: "Regular Check-up", status: "Cancelled" },
        { id: "a7", appointmentId: "APT-2611-340", time: "02:45 PM", patientName: "Melchora Aquino", patientId: "STUD-2022-0987", email: "melchora.aquino@example.com", service: "Lab Result Review", status: "Completed" },
        { id: "a8", appointmentId: "APT-2612-341", time: "04:10 PM", patientName: "Lapu-Lapu", patientId: "STUD-2024-0011", email: "lapu.lapu@example.com", service: "Dental Consultation", status: "Cancelled" },
        { id: "a9", appointmentId: "APT-2613-342", time: "11:00 AM", patientName: "Francisco Balagtas", patientId: "STUD-2024-0999", email: "balagtas@example.com", service: "Regular Check-up", status: "Completed" },
        { id: "a10", appointmentId: "APT-2614-343", time: "02:00 PM", patientName: "Gregoria de Jesus", patientId: "STUD-2023-0777", email: "gregoria@example.com", service: "Vaccination", status: "Completed" }
    ];
    ...
    function renderAppointmentsTable() {
        ...
        document.getElementById('totalCount').innerText = appointmentsList.length;
        
        if (pageItems.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="text-center py-5"><div class="empty-state"><i class="fa-solid fa-calendar-xmark fa-3x mb-3 opacity-50"></i><p class="mb-0">No appointment history found.</p><small class="text-muted">Try adjusting your search.</small></div></td></tr>`;
            renderPaginationControls();
            return;
        }
        
        // Table columns: Student Details | Email | Appointment ID (NEW) | Service Type | Status | Time | Action
            ...
            return `<tr data-id="${apt.id}">
                ...
                <td style="text-align: center;">
                    <div class="action-buttons">
                        <button class="action-btn" onclick="openEditModal('${apt.id}')" title="Edit Appointment"><i class="fa-regular fa-pen-to-square"></i></button>
                        <button class="action-btn" onclick="confirmDeleteAppointment('${apt.id}', '${safePatientName}')" title="Delete Appointment"><i class="fa-regular fa-trash-can"></i></button>
                    </div>
                </td>
              </tr>`;
        }).join('');
    ...
    window.openEditModal = function(id) {
        ...
        document.getElementById('editEmail').value = apt.email || '';
        document.getElementById('editAppointmentIdDisplay').value = apt.appointmentId;
        document.getElementById('editService').value = apt.service;
        document.getElementById('editStatus').value = apt.status;
        
        let timeValue = apt.time;
        if (timeValue.includes('AM') || timeValue.includes('PM')) {
            let [timePart, period] = timeValue.split(' ');
            let [hour, minute] = timePart.split(':');
            let hour24 = parseInt(hour);
            if (period === 'PM' && hour24 !== 12) hour24 += 12;
            if (period === 'AM' && hour24 === 12) hour24 = 0;
            timeValue = `${hour24.toString().padStart(2, '0')}:${minute}`;
        }
    ...
    function saveEditChanges() {
        ...
        if (!patientName || !patientId || !timeRaw || !service) {
            showToastMessage("Please fill all required fields.", "danger");
            return;
        }
        ...
        let displayTime = `${hour12.toString().padStart(2, '0')}:${minute} ${period}`;
        
        const index = appointmentsList.findIndex(a => a.id === id);
        if (index !== -1) {
            ...
            showToastMessage("Appointment updated successfully!", "success");
        }
    }

    window.confirmDeleteAppointment = function(id, name) {
        currentDeleteId = id;
        document.getElementById('deleteAppointmentName').innerText = name;
        deleteModalInstance.show();
    };
    
    function executeDelete() {
        if (currentDeleteId) {
            const deletedAppointment = appointmentsList.find(a => a.id === currentDeleteId);
            appointmentsList = appointmentsList.filter(a => a.id !== currentDeleteId);
            refreshTable();
            deleteModalInstance.hide();
            currentDeleteId = null;
            showToastMessage(`${deletedAppointment?.patientName || "Appointment"} deleted permanently.`, "danger");
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        editModalInstance = new bootstrap.Modal(document.getElementById('editAppointmentModal'), { backdrop: 'static', keyboard: false });
