...
<head>
    ...
    <title>Student Appointments | PUPBC CareLink</title>
...
<body>
    ...
    <div class="floating-form-container">
        <nav class="nav-header-tab">
            <div class="nav-tabs-inline" id="appointmentTabs">
                <a href="studappointmentsonline.php" data-tab="online" class="nav-link-tab">
                    <span>Online Appointment</span>
                </a>
                <a href="studappointmentscancel.php" data-tab="cancel" class="nav-link-tab">
                    <span>Cancel Appointments</span>
                </a>
                <a href="studappointmentshistory.php" data-tab="history" class="nav-link-tab">
        ...
        <div class="profile-body">
            <iframe src="about:blank" name="appointment-frame" id="appointment-frame"></iframe>
        </div>        
    </div>
    ...
    <script>
        ...
        if (window.self === window.top) {
            ...
            const sub = urlParams.get('subpage') || 'studappointmentsonline.php';
        ...
        document.addEventListener('DOMContentLoaded', function() {
             ...
             * Ito ang mag-u-update ng URL bar ng dashboard agad-agad.
             */
            function switchTab(tabId) {
                const mapping = {
                    'online': 'studappointmentsonline.php',
                    'cancel': 'studappointmentscancel.php',
                    'history': 'studappointmentshistory.php'
                };

                const targetSrc = mapping[tabId] || 'studappointmentsonline.php';
                ...
                // URL SYNC: Pinapakita ang ?page=studappointments.php&subpage=... sa dashboard
                try {
                    if (window.parent !== window.self) {
                        const newURL = window.parent.location.origin + window.parent.location.pathname + 
                                       '?page=studappointments.php&subpage=' + targetSrc;
            ...
            try {
                if (window.parent !== window.self) {
                    ...
                    if (parentSubpage) {
                        if (parentSubpage.includes('online')) activeSub = 'online';
                        else if (parentSubpage.includes('cancel')) activeSub = 'cancel';
                        else if (parentSubpage.includes('history')) activeSub = 'history';
                    }
                }
            } catch(e) { activeSub = 'online'; }
            ...
            /**
             * 5. IFRAME-TO-PARENT SYNC (AUTO-UPDATE ON NAV)
             */
            iframe.addEventListener('load', function() {
                if (isSwitchingTab) return;
                try {
                    const iframeLoc = iframe.contentWindow.location.pathname.split('/').pop();
                    let currentId = '';
                    
                    if (iframeLoc.includes('online')) currentId = 'online';
                    else if (iframeLoc.includes('cancel')) currentId = 'cancel';
                    else if (iframeLoc.includes('history')) currentId = 'history';
                    
                    if (currentId) {
                        tabs.forEach(t => {
                            t.classList.remove('active');
                            if (t.getAttribute('data-tab') === currentId) t.classList.add('active');
                        });
                        
                        if (window.parent !== window.self) {
                            const newURL = window.parent.location.origin + window.parent.location.pathname + 
                                           '?page=studappointments.php&subpage=' + iframeLoc;
                            window.parent.history.replaceState({ path: newURL }, '', newURL);
                        }
                    }
                } catch(e) { }
